...
async function getSubs(): Promise<Record<string, any>> {
	return {
		'Mega Easy': await stageSubmission.find({
			accepted: true,
			difficulty: 'Mega Easy',
		}),
		Easy: await stageSubmission.find({
			accepted: true,
			difficulty: 'Easy',
		}),
		Medium: await stageSubmission.find({
			accepted: true,
			difficulty: 'Medium',
		}),
		Hard: await stageSubmission.find({
			accepted: true,
			difficulty: 'Hard',
		}),
		Extreme: await stageSubmission.find({
			accepted: true,
			difficulty: 'Extreme',
		}),
	};
	...
	return Math.round(
		Math.round(
			Math.max(
				5000 *
					(baseline /
						(Subs[currentSub?.difficulty as string].length +
							(currentSub?.accepted ? 0 : 1))),
				1000
			)
		) *
			((currentSub?.paymentPercentage || 0) / 100)
	);
}

async function sendNextStagePayments(
	paymentInfoChannel: TextChannel
): Promise<void> {
	const Subs = await getSubs();
	...
	for (const key in Subs) {
		const maxAmount = Math.round(
			Math.max(5000 * (baseline / (Subs[key].length + 1)), 1000)
		);
		paymentInfoEmbed.addField(
			key + ' Stages',
			`> Total: ${Subs[key].length}\n> Max Payment: ${maxAmount} Robux`
		);
	}

	paymentInfoEmbed.setFooter({
		text: 'Note: maximum payment is based on demand',
	});
...
export async function execute(reaction: MessageReaction, user: User) {
	if (reaction.partial) {
		reaction
			...
			.catch(err => {
				log({
					logger: 'reaction',
					content: `Something went wrong fetching the reaction: ${err}`,
					level: 'error',
				});
				return;
			});
		...
		stageSubmission.findById(
			...
			async (err, submission) => {
				if (err) {
					...
					const acceptanceMessage = await (
							acceptedStagesChannel as TextChannel
					).send({
					...
					await sendNextStagePayments(
							paymentInfoChannel as TextChannel
					);
				}
				else {
					...
					user
						.send(
							'Missing one of payment percentage or difficulty!'
						)
						.catch(err => {
							log({
								logger: 'reaction',
								content: `Something went wrong DMing the reaction sender: ${err}`,
								level: 'error',
							});
						});
				}
	...
	case 'Paid': {
		...
		stageSubmission.findOneAndUpdate(
			{ acceptanceMessageId: reaction.message.id },
			{ payedOut: true },
			{ new: true },
			async (err, submission) => {
				if (err) {
					...
					const paymentLogChannel =
							await reaction.message.guild?.channels.fetch(
								constants['paymentLogChannel']
							);
					...
					const paymentLogEmbed = new MessageEmbed()
						.setColor('#00ff77')
						.setDescription(
							`<@${submission?.authorId}> has been paid ${submission?.paymentRequired} Robux for their stage.\nStage ID: \`${submission?._id}\``
						);
	...
	case 'Verify': {
		...
		await (acceptedStagesChannel as TextChannel).messages
			.fetch(submission?.acceptanceMessageId as string)
			.then(m => {
			...
			.catch(e => {
				log({
					logger: 'reaction',
					content: `Failed to reacted to accepted stage message: ${e}`,
					level: 'error',
				});
			});
		break;
